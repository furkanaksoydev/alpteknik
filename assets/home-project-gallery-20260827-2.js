...
(function () {
  ...
  function init() {
    ...
    const candidates = valves.filter(product => product.images?.[0]).map(product => ({
      ...
      href: `kataloglar.html?category=vanalar&brand=${encodeURIComponent(brandOf(product))}&group=${encodeURIComponent(product.groupId)}&product=${encodeURIComponent(product.id)}`,
